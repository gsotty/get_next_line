...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line_test4.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gsotty <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/11/23 03:09:20 by gsotty            #+#    #+#             */
/*   Updated: 2016/11/23 16:13:07 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_while(t_file *file, const int fd)
{
	...
	while (file->buf[0][z++] != '\0')
	{
		...
		if (file->buf[0][z + 1] == '\0')
		{
			...
			while (file->fd[x] != fd)
				x++;
			ret = read(file->fd[x], tmp2, BUFF_SIZE);
			tmp2[ret] = '\0';
...
static t_file	*ft_malloc(const int fd, t_file *file)
{
	...
	while (file->fd[x] != fd && x <= file->cont)
		x++;
	if (file->fd[x] != fd)
	{
		fd_tmp = file->fd;
